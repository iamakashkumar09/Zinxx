...
def _generate(model, model_config, prompt: str, seconds: float) -> AudioSegment:
    ...
    try:
        # This model's architecture always conditions on inpaint_mask/inpaint_masked_input,
        # even for plain generation — generate_diffusion_cond_inpaint supplies the
        # (zeroed, i.e. "generate everything from scratch") defaults; generate_diffusion_cond
        # does not, and raises KeyError: 'inpaint_mask'.
        from stable_audio_tools.inference.generation import generate_diffusion_cond_inpaint as generate_diffusion_cond
    except ImportError as e:
        raise RuntimeError(
            "stable-audio-tools is not installed. Run: pip install stable-audio-tools"
        ) from e
    ...
    with torch.inference_mode():
        output = generate_diffusion_cond(
            ...
            # This model's diffusion_objective is "rf_denoiser" (rectified flow), which only
            # supports euler/rk4/dpmpp/pingpong samplers — dpmpp-3m-sde and sigma_min/max are
            # k-diffusion-only options and raise ValueError here. Let it use its pingpong default.
            device=DEVICE,
